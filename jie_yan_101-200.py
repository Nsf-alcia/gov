...
def getDataExisted(Area):
    ...
    conn = pymysql.connect(host='61.145.61.72', user='greentest', password='test@2018', port=3306, db='greentest',
                           charset='utf8')
    cursor = conn.cursor()
    try:
        cursor.execute(sql)
...
def sendDataToDb(dataToDb):
    conn = pymysql.connect(host='61.145.61.72', user='greentest', password='test@2018', port=3306, db='greentest', charset='utf8')
    cur = conn.cursor()
    if dataToDb != None:
        try:
            print(dataToDb[0])
...
def getDataFromUrl(link, AdministrativeNumber):
    ...
    if a_word == None and a_href == None :  # 详情 页面上内容 即不是 下载文档 也不是 在线pdf 文件。
        ...
        if '揭阳市OA系统（印章）' in content:
            jieshu = content.split('揭阳市OA系统（印章）')[-1]
        ...
        if CreateDate.strip() == '':
            if '揭阳市OA系统' in content:
                jieshu = content.split('揭阳市OA系统')[-1]
                if '年' in jieshu and '月' in jieshu and '日' in jieshu:
                    match = pattern.search(jieshu)
                    CreateDate = match.group().replace('年', '-').replace('月', '-').replace('日', '')
...
def getLinks():
    for i in range(101,201): # 1 - 332
        url = 'http://oa.jieyang.gov.cn/OpenInfoRight.action?pageNum='+str(i)+'&pageSize=10&depCode=007026553'
